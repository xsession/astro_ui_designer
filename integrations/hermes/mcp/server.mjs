...
rl.on('line',async line=>{let m;try{m=JSON.parse(line)}catch{return}try{
  if(m.method==='initialize')return reply(m.id,{protocolVersion:m.params?.protocolVersion||'2025-11-25',capabilities:{tools:{}},serverInfo:{name:'astro-ui-designer',version:'2.19.0'}});
