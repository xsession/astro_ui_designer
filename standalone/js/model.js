...
export const deepClone = (value) => value === undefined ? undefined : JSON.parse(JSON.stringify(value));
...
export function createNode(type, overrides = {}) {
  ...
  return {
    ...
    design: deepClone(overrides.design || {constraints:{horizontal:'left',vertical:'top'},effects:{fills:[],strokes:[],shadows:[],blurs:[],blendMode:'normal'},exportPresets:[],interactions:[],fixedOnScroll:false,clipContent:false,aspectLocked:false,vector:{kind:'none',path:'',booleanOperation:'none'},libraryRef:'',manualLayout:{sizingX:'fixed',sizingY:'fixed',rotation:0,flipX:false,flipY:false,minWidth:'',maxWidth:'',minHeight:'',maxHeight:''}}),
...
export function createProject() {
  ...
  return {
    schemaVersion: 9, id: makeId('project'), name: 'astro-ui-project',
    settings: {
    ...
    design: {guides:[],comments:[],flows:[],libraries:[],review:{showResolved:false},inspect:{showMeasurements:true,showCode:true},interchange:{lastImport:null,lastExport:null}},
    composition: {version:1,globalVariants:[],activeGlobalVariants:{},mixins:[],globalContexts:[],queries:[],templates:[],refactors:{history:[]}},
    manualLayout: {version:1,settings:{rulers:true,snapEnabled:true,snapGrid:true,snapGuides:true,snapGeometry:true,snapDistance:6,showSmartGuides:true,measureOnAlt:true,showSpacing:false,xray:false,bigNudge:8,fineNudge:1},layoutGuides:{},gridEdit:{visible:true}},
    simulation: {version:1,settings:{scope:'project',viewport:'desktop',showHotspots:true,showState:true,reducedMotion:false,autoDelays:true},fixtures:[]},
  };
}
...
export function moveNode(root,id,newParentId,index=null) { const node=findNode(root,id),oldParent=findParent(root,id); if(!node||!oldParent) return false; const oldIndex=oldParent.children.findIndex(c=>c.id===id); oldParent.children.splice(oldIndex,1); try { insertNode(root,newParentId,node,index); return true; } catch(e) { oldParent.children.splice(Math.max(0,oldIndex),0,node); throw e; } }
...
export function migrateProject(project) {
  ...
  if(p.schemaVersion<5){ p.design ||= {guides:[],comments:[],flows:[],libraries:[],review:{showResolved:false},inspect:{showMeasurements:true,showCode:true},interchange:{lastImport:null,lastExport:null}}; p.schemaVersion=5; }
  if(p.schemaVersion<6){ p.storybook ||= {version:1,settings:{defaultViewport:'desktop',defaultBackground:'light',direction:'ltr',grid:false,outline:false,measure:false,watch:false,coverage:false},tags:[],results:{},visualBaselines:{},docs:{title:`${p.name||'Project'} Component Library`,description:'Generated component documentation.'}}; p.schemaVersion=6; }
  if(p.schemaVersion<7){ p.composition ||= {version:1,globalVariants:[],activeGlobalVariants:{},mixins:[],globalContexts:[],queries:[],templates:[],refactors:{history:[]}}; p.schemaVersion=7; }
  if(p.schemaVersion<8){ p.manualLayout ||= {version:1,settings:{rulers:true,snapEnabled:true,snapGrid:true,snapGuides:true,snapGeometry:true,snapDistance:6,showSmartGuides:true,measureOnAlt:true,showSpacing:false,xray:false,bigNudge:8,fineNudge:1},layoutGuides:{},gridEdit:{visible:true}}; p.schemaVersion=8; }
  if(p.schemaVersion<9){ p.simulation ||= {version:1,settings:{scope:'project',viewport:'desktop',showHotspots:true,showState:true,reducedMotion:false,autoDelays:true},fixtures:[]}; p.schemaVersion=9; }
  p.simulation ||= {version:1,settings:{scope:'project',viewport:'desktop',showHotspots:true,showState:true,reducedMotion:false,autoDelays:true},fixtures:[]};
  for(const c of p.components||[]){ c.props ||= []; c.variants ||= []; c.stories ||= []; }
  for(const {node} of collectProjectNodes(p)){ if(node.type==='componentInstance'){node.props ||= {}; node.props.propValues ||= {};} node.bindings ||= {}; node.meta ||= {domId:'',className:'',slot:'',locked:false,hidden:false}; node.meta={domId:'',className:'',slot:'',locked:false,hidden:false,sourceOwnership:'designer',exposed:{content:true,style:false,structure:false,actions:false},...node.meta}; node.actions ||= []; node.children ||= []; node.style ||= {base:{}}; node.style.base ||= {}; node.variant ||= ''; node.componentState ||= 'default'; node.states ||= []; node.containerRules ||= []; node.timeline ||= { duration: 500, delay: 0, easing: 'ease', iterations: 1, direction: 'normal', fill: 'both', playbackRate: 1, reducedMotion: 'disable', engine: 'auto', trigger: 'manual', scroll: { timeline: 'view', source: 'nearest', axis: 'block', rangeStart: 'entry 0%', rangeEnd: 'cover 100%' }, tracks: [] }; node.timeline={...{ duration: 500, delay: 0, easing: 'ease', iterations: 1, direction: 'normal', fill: 'both', playbackRate: 1, reducedMotion: 'disable', engine: 'auto', trigger: 'manual', scroll: { timeline: 'view', source: 'nearest', axis: 'block', rangeStart: 'entry 0%', rangeEnd: 'cover 100%' }, tracks: [] },...node.timeline,scroll:{...{ duration: 500, delay: 0, easing: 'ease', iterations: 1, direction: 'normal', fill: 'both', playbackRate: 1, reducedMotion: 'disable', engine: 'auto', trigger: 'manual', scroll: { timeline: 'view', source: 'nearest', axis: 'block', rangeStart: 'entry 0%', rangeEnd: 'cover 100%' }, tracks: [] }.scroll,...(node.timeline.scroll||{})}}; node.visibilityCondition ||= ''; node.dataContext ||= {}; node.design ||= {constraints:{horizontal:'left',vertical:'top'},effects:{fills:[],strokes:[],shadows:[],blurs:[],blendMode:'normal'},exportPresets:[],interactions:[],fixedOnScroll:false,clipContent:false,aspectLocked:false,vector:{kind:'none',path:'',booleanOperation:'none'},libraryRef:''}; node.design.constraints ||= {horizontal:'left',vertical:'top'}; node.design.effects ||= {fills:[],strokes:[],shadows:[],blurs:[],blendMode:'normal'}; node.design.exportPresets ||= []; node.design.interactions ||= []; node.design.manualLayout ||= {sizingX:'fixed',sizingY:'fixed',rotation:0,flipX:false,flipY:false,minWidth:'',maxWidth:'',minHeight:'',maxHeight:''}; node.design.manualLayout={sizingX:'fixed',sizingY:'fixed',rotation:0,flipX:false,flipY:false,minWidth:'',maxWidth:'',minHeight:'',maxHeight:'',...node.design.manualLayout}; node.cssStates ||= {hover:{},focus:{},focusVisible:{},active:{},disabled:{}}; for(const key of ['hover','focus','focusVisible','active','disabled']) node.cssStates[key] ||= {}; node.cssVariables ||= {}; }
  return p;
}
