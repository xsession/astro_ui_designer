...
for(const backend of ['astro','react','vue','svelte','vanilla-js','tkinter','nicegui','lvgl']){const files=generateNeutralBackend(doc,backend,{componentName:'Demo'});assert.ok(Object.keys(files).length>0,backend);assert.ok(Object.values(files).every(v=>typeof v==='string'));}
