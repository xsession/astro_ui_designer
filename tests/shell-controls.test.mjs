...
try {
  ...
  const staticIds = [
    ...
    'breakpoint-select','breakpoints-btn','left-tabs-more','right-tabs-more','bottom-tabs-more','bottom-toggle-btn','design-mode-btn','split-mode-btn','code-mode-btn','preview-mode-btn','zoom-out-btn','fit-btn','zoom-in-btn','workspace-btn','live-preview-btn','command-btn','export-btn','file-open','asset-open'
  ];
  ...
  const handlerIds = staticIds.filter(x=>!['file-open','asset-open'].includes(x));
  ...
  const rightExpect={properties:'#obj-name',layout:'[id^="s-"]',actions:'#add-action',bindings:'[data-binding]',code:'#code-file',variants:'#add-node-state',data:'#source-ownership'};
  ...
  // Bottom dock tabs.
  for (const tab of ['problems','objects','state','animation','tokens','actions','content','locales','tests','audit','git','integrations','console']) {
  ...
  // Design/preview and zoom toolbar.
  ...
  console.log(`shell-controls.test: OK (${staticIds.length} static controls, 8 menus, 5 left tabs, 7 right tabs, 13 bottom tabs)`);
} finally {
  await browser.close();
}
