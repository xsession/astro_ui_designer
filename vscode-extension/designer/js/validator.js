...
export function validateProject(project) {
  ...
  const mixinIds=new Set((project.composition?.mixins||[]).map(x=>x.id)), queryIds=new Set((project.composition?.queries||[]).map(x=>x.id)), contextIds=new Set((project.composition?.globalContexts||[]).map(x=>x.id)), globalVariantIds=new Set((project.composition?.globalVariants||[]).map(x=>x.id));
  const compNames=new Set(); for(const q of project.composition?.queries||[]){if(compNames.has(`q:${q.name}`))add('warning','CMPQ01',`Duplicate query name: ${q.name}`);compNames.add(`q:${q.name}`);if(['http','graphql'].includes(q.kind)&&!q.source)add('warning','CMPQ02',`Query ${q.name} has no endpoint.`);}
  ...
  for(const item of collectProjectNodes(project)){
    ...
    for(const id of node.composition?.mixins||[])if(!mixinIds.has(id))add('warning','MIX01',`Missing style mixin: ${id}`,node.id,owner.id);
    for(const b of Object.values(node.composition?.queryBindings||{}))if(b?.queryId&&!queryIds.has(b.queryId))add('error','CMPQ10',`Missing query binding: ${b.queryId}`,node.id,owner.id);
    for(const b of Object.values(node.composition?.contextBindings||{}))if(b?.contextId&&!contextIds.has(b.contextId))add('error','CTX10',`Missing global context: ${b.contextId}`,node.id,owner.id);
    for(const r of node.composition?.globalVariantStyles||[])if(r.groupId&&!globalVariantIds.has(r.groupId))add('warning','GV10',`Missing global variant group: ${r.groupId}`,node.id,owner.id);
