...
function scanWorkspace(root){root=safeRoot(root);const files=[];function rec(dir){for(const e of fs.readdirSync(dir,{withFileTypes:true})){if(['node_modules','.git','dist','.astro'].includes(e.name))continue;const p=path.join(dir,e.name);if(e.isDirectory())rec(p);else if((/\.(astro|tsx?|jsx?|mjs|cjs|vue|svelte|css|scss|sass|less|json|html?|pyi?|c|h|cpp|hpp|qml|qmltypes|qmlproject|qrc)$/i.test(e.name)||['CMakeLists.txt','qmldir','qtquickcontrols2.conf'].includes(e.name)))files.push({relativePath:path.relative(root,p).split(path.sep).join('/'),size:fs.statSync(p).size})}}rec(root);return {rootPath:root,files}}
